/** Execute `gjc memory search` through the public deterministic retrieval operation. */
import {
	MEMORY_EXIT_CODES,
...
export type MemorySearchCommandResult = SearchResult;
export interface MemorySearchCommandOptions {
	readonly json?: boolean;
	readonly explain?: boolean;
}

export type MemorySearchCommandFlags = MemorySearchCommandOptions;
...
function validOptions(options: MemorySearchCommandOptions): MemoryResult<MemorySearchCommandOptions> {
	if (options === null || typeof options !== "object" || Array.isArray(options)) {
		return invalid("memory search options must be an object");
	}
	if (options.json !== undefined && typeof options.json !== "boolean") {
		return invalid("memory search json option must be boolean");
	}
	if (options.explain !== undefined && typeof options.explain !== "boolean")
		return invalid("memory search explain option must be boolean");
	return { ok: true, value: options };
}

function normalizeInput(input: SearchInput): MemoryResult<SearchInput> {
	if (input === null || typeof input !== "object" || Array.isArray(input)) {
		return invalid("memory search input must be an object");
	}
	if (typeof input.query !== "string" || input.query.normalize("NFC").trim().length === 0) {
		return invalid("memory search query must be a non-empty string");
	}
	...
	if (
		...
		return invalid("memory search query contains a control character");
	}

	let scopes: readonly MemoryScopeKind[] | undefined;
	if (input.scopes !== undefined) {
		if (!Array.isArray(input.scopes) || input.scopes.length === 0)
			return invalid("memory search scopes must not be empty");
		const normalizedScopes: MemoryScopeKind[] = [];
		for (const scope of input.scopes) {
			if (typeof scope !== "string") return invalid("memory search scope must be a string");
			const normalized = scope.normalize("NFC").toLowerCase();
			if (!(MEMORY_SCOPES as readonly string[]).includes(normalized)) {
				return invalid(`memory search scope is unsupported: ${scope}`);
			}
	...
	if (input.limit !== undefined && (!Number.isSafeInteger(input.limit) || input.limit <= 0)) {
		return invalid("memory search limit must be a positive integer");
	}
	if (input.complete !== undefined && typeof input.complete !== "boolean") {
		return invalid("memory search complete option must be boolean");
	}
	if (input.intent !== undefined && typeof input.intent !== "string") {
		return invalid("memory search intent must be a string");
	}
	if (input.explain !== undefined && typeof input.explain !== "boolean") {
		return invalid("memory search explain input must be boolean");
	}
...
function renderHuman(result: SearchResult): string {
	const lines = [`query: ${result.query}`, `truncated: ${result.truncated ? "yes" : "no"}`];
...
/** Search ranked citations and render deterministic human or versioned JSON output. */
export async function runSearchCommand(
	...
	const result = await search(environment, {
		...checkedInput.value,
